...
func TestRenamePlaylistSendsUpdatePlaylistWithName(t *testing.T) {
	var got url.Values
	body := `{"subsonic-response":{"status":"ok","version":"1.16.1"}}`
	a := newRecordingAdapter(t, http.StatusOK, body, &got)

	if err := a.RenamePlaylist(context.Background(), "pl-7", "Summer Hits"); err != nil {
		t.Fatal(err)
	}
	if got.Get("playlistId") != "pl-7" {
		t.Errorf("playlistId = %q, want pl-7", got.Get("playlistId"))
	}
	if got.Get("name") != "Summer Hits" {
		t.Errorf("name = %q, want Summer Hits", got.Get("name"))
	}
}

func TestRenamePlaylistSurfacesUpstreamError(t *testing.T) {
	var got url.Values
	body := `{"subsonic-response":{"status":"failed","version":"1.16.1","error":{"code":70,"message":"Playlist not found"}}}`
	a := newRecordingAdapter(t, http.StatusOK, body, &got)

	if err := a.RenamePlaylist(context.Background(), "missing", "X"); err == nil {
		t.Fatal("expected error from failed subsonic status")
	}
}

func TestRemovePlaylistTracksSendsUpdatePlaylistWithIndices(t *testing.T) {
	var got url.Values
	body := `{"subsonic-response":{"status":"ok","version":"1.16.1"}}`
	a := newRecordingAdapter(t, http.StatusOK, body, &got)

	if err := a.RemovePlaylistTracks(context.Background(), "pl-1", []int{0, 3, 7}); err != nil {
		t.Fatal(err)
	}
	if got.Get("playlistId") != "pl-1" {
		t.Errorf("playlistId = %q, want pl-1", got.Get("playlistId"))
	}
	idxs := got["songIndexToRemove"]
	if len(idxs) != 3 || idxs[0] != "0" || idxs[1] != "3" || idxs[2] != "7" {
		t.Fatalf("songIndexToRemove = %v, want [0 3 7]", idxs)
	}
}

func TestRemovePlaylistTracksSurfacesUpstreamError(t *testing.T) {
	var got url.Values
	body := `{"subsonic-response":{"status":"failed","version":"1.16.1","error":{"code":70,"message":"Playlist not found"}}}`
	a := newRecordingAdapter(t, http.StatusOK, body, &got)

	if err := a.RemovePlaylistTracks(context.Background(), "missing", []int{0}); err == nil {
		t.Fatal("expected error from failed subsonic status")
	}
}

func TestDeletePlaylistHitsDeletePlaylistEndpoint(t *testing.T) {
	var got url.Values
	body := `{"subsonic-response":{"status":"ok","version":"1.16.1"}}`
	a := newRecordingAdapter(t, http.StatusOK, body, &got)

	if err := a.DeletePlaylist(context.Background(), "pl-42"); err != nil {
		t.Fatal(err)
	}
	if got.Get("id") != "pl-42" {
		t.Errorf("id = %q, want pl-42", got.Get("id"))
	}
}

func TestDeletePlaylistSurfacesUpstreamError(t *testing.T) {
	var got url.Values
	body := `{"subsonic-response":{"status":"failed","version":"1.16.1","error":{"code":70,"message":"Playlist not found"}}}`
	a := newRecordingAdapter(t, http.StatusOK, body, &got)

	if err := a.DeletePlaylist(context.Background(), "missing"); err == nil {
		t.Fatal("expected error from failed subsonic status")
	}
}
